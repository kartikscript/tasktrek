...
const RenderTitle=({title,route,icon}:{title:string,route:string, icon:React.ReactNode})=>{
  ...
  return(
    <Link href={route}
     className={`${isActive && 'bg-neutral-800 text-purple-400'} tracking-wide flex gap-4 items-center px-6 py-3 rounded-lg text-purple-100 hover:text-purple-400 transition-all duration-200`}
    >
      {icon}{title}
    </Link>
  )
...
const SideBar = () => {
  return (
    <aside className='h-screen p-6 sticky top-0 left-0'>
      <div className=' h-full w-72 p-6 bg-neutral-900 text-neutral-50 font-IBMPlex rounded-2xl shadow-md shadow-black/40'>
        <h2 className='font-robotoSerif font-bold text-2xl flex items-center gap-3 mx-6 my-8 shadow-md bg-gradient-to-tr from-purple-800 via-purple-400 to-purple-900 bg-clip-text text-transparent'><SiTask className='size-10 text-purple-300'/> TaskTrek</h2>     
